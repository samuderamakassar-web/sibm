...
export default function PaketPage() {
  ...
  const handleDiambil = async (id: string, namaPenerima: string) => {
    const yakin = await confirm({
      ...
      message: `Konfirmasi: Serahkan paket ini kepada ${namaPenerima}?`,
    ...
    try {
      ...
      await updateDoc(paketRef, { waktu_diambil: serverTimestamp(), status: "Sudah Diambil" });
    } catch (error) {
  ...
  return (
    <div style={{ backgroundColor: "#f8fafc", minHeight: "100vh", fontFamily: "'Inter', sans-serif", paddingBottom: "50px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "15px 20px", background: "white", borderBottom: "1px solid #e2e8f0", position: "sticky", top: 0, zIndex: 50 }}>
        ...
        <div style={{ background: "#ebf8ff", color: "#3182ce", padding: "8px 15px", borderRadius: "8px", fontSize: "12px", fontWeight: "bold", border: "1px solid #bee3f8" }}>👮 {picName}</div>
      </div>
      ...
      <div style={{ maxWidth: "1200px", margin: "-20px auto 0", padding: "0 20px", position: "relative", zIndex: 10, display: "flex", gap: "25px", flexWrap: "wrap", alignItems: "flex-start" }}>
        <Card style={{ flex: "1 1 350px" }}>
          ...
          {isSuccess && (
            <div style={{ background: "#f0fff4", color: "#22543d", padding: "12px", borderRadius: "12px", marginTop: "20px", fontSize: "13px", fontWeight: "bold", border: "1px solid #c6f6d5", display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "16px", marginTop: "20px" }}>
            ...
            <div style={{ position: "relative" }}>
              <Input
                ...
                required
                style={{ border: "2px solid #63b3ed", background: "#ebf8ff", color: "#2b6cb0", fontWeight: "bold" }}
              />
                ...
                <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "white", border: "1px solid #e2e8f0", borderRadius: "10px", marginTop: "5px", zIndex: 50, maxHeight: "200px", overflowY: "auto", boxShadow: "0 10px 15px rgba(0,0,0,0.1)" }}>
                  {filteredKaryawan.length > 0 ? (
                    filteredKaryawan.map((emp, idx) => (
                      <div
                        key={idx}
                        onClick={() => pilihKaryawan(emp)}
                        style={{ padding: "12px", borderBottom: "1px solid #edf2f7", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}
                      >
        ...
        <Card style={{ flex: "2 1 500px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px", flexWrap: "wrap", gap: "10px" }}>
            ...
            <input
              ...
              style={{ padding: "12px 16px", borderRadius: "20px", border: "1px solid #cbd5e0", background: "#f8fafc", outline: "none", fontSize: "14px", width: "250px" }}
            />
          </div>
            ...
            <TBody>
              {paketTerfilter.length === 0 ? (
                ...
                paketTerfilter.map((pkt) => (
                  <Tr key={pkt.id} style={{ background: pkt.status === "Sudah Diambil" ? "#fbfcfd" : "white" }}>
                    ...
                    <Td style={{ color: "#2b6cb0", fontWeight: "900" }}>{pkt.penerima}</Td>
                    <Td style={{ color: "#718096" }}>{pkt.kurir}</Td>
                    <Td style={{ color: "#4a5568" }}>{formatWaktu(pkt.waktu_diterima)}</Td>
                    <Td style={{ textAlign: "center" }}>
                      {pkt.status === "Belum Diambil" ? (
                        <Button fullWidth={false} variant="warning" style={{ padding: "8px 14px", fontSize: "11px" }} onClick={() => handleDiambil(pkt.id, pkt.penerima)}>
                          Serahkan ➔
                        </Button>
                      ) : (
                        <div>
