    ...
    <div style={{ backgroundColor: "#f8fafc", minHeight: "100vh", fontFamily: "'Inter', sans-serif" }}>
      ...
      <style jsx global>{`
        ...
        @media print {
          ...
          .detail-section { page-break-before: always; margin-top: 20px; }
      ...
      <div className="no-print" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "15px 30px", background: "white", borderBottom: "1px solid #e2e8f0", position: "sticky", top: 0, zIndex: 50 }}>
        ...
        <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "5px", background: "#f1f5f9", padding: "4px 8px", borderRadius: "8px", border: "1px solid #cbd5e0" }}>
          ...
          <button onClick={handlePrint} style={{ background: "#2b6cb0", color: "white", padding: "8px 15px", borderRadius: "8px", fontSize: "12px", fontWeight: "bold", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: "5px" }}>
            🖨️ Cetak PDF Laporan
          </button>
        </div>
      </div>
      ...
      <div className="print-area" style={{ maxWidth: "1000px", margin: "30px auto", padding: "0 20px", paddingBottom: "50px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "3px solid #e53e3e", paddingBottom: "15px", marginBottom: "25px" }}>
          <div>
              ...
              {helpdeskList.length > 0 ? (
                <table>
                  ...
                  <tbody>
                    {helpdeskList.map(h => (
                      <tr key={h.id}>
                        <td style={{ whiteSpace: "nowrap" }}>{formatTgl(h.waktu_lapor)}</td>
                        <td>{h.nama_pelapor}</td>
                        <td>{h.lokasi}</td>
                        <td>{h.deskripsi}</td>
                        <td style={{ fontWeight: "bold", color: h.status === "Selesai" ? "#38a169" : "#dd6b20" }}>{h.status}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              {sboList.length > 0 ? (
                <table>
                  ...
                  <tbody>
                    {sboList.map(s => (
                      <tr key={s.id}>
                        <td style={{ whiteSpace: "nowrap" }}>{formatTgl(s.waktu_lapor)}</td>
                        <td>{s.nama_pelapor}</td>
                        <td>{s.kategori_temuan}</td>
                        <td><b>{s.lokasi}</b> - {s.detail_temuan}</td>
                        <td style={{ fontWeight: "bold", color: s.status_temuan === "Open" ? "#e53e3e" : "#38a169" }}>{s.status_temuan}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              {atkList.length > 0 ? (
                <table>
                  ...
                  <tbody>
                    {atkList.map(a => (
                      <tr key={a.id}>
                        <td style={{ whiteSpace: "nowrap" }}>{formatTgl(a.waktu_request)}</td>
                        <td>{a.resi}</td>
                        <td>{a.nama_pemohon} ({a.departemen})</td>
                        {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                        <td>{a.items ? a.items.map((i: any) => `${i.nama_barang} (${i.jumlah})`).join(", ") : "-"}</td>
                        <td style={{ fontWeight: "bold", color: a.status?.includes("Selesai") ? "#38a169" : "#d69e2e" }}>{a.status}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              {overtimeList.length > 0 ? (
                <table>
                  ...
                  <tbody>
                    {overtimeList.map(o => (
                      <tr key={o.id}>
                        <td style={{ whiteSpace: "nowrap" }}>{formatTgl(o.waktu_request)}</td>
                        <td>{o.periode ? o.periode : "Gedung (Harian)"}</td>
                        <td>{o.nama_pemohon}</td>
                        <td>{o.departemen}</td>
                        <td style={{ fontWeight: "bold", color: o.status === "Approved" ? "#38a169" : o.status === "Rejected" ? "#e53e3e" : "#d69e2e" }}>{o.status}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
