...
export default function ManajemenKaryawanPage() {
  ...
  const handleUploadCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!window.confirm("Pastikan format file CSV Anda: Nama, Departemen, Plat Kendaraan. Lanjutkan import?")) {
    ...
    reader.onload = async (event) => {
      ...
      try {
        ...
        alert("Gagal memproses file CSV. Pastikan format kolom dipisahkan dengan koma (,).");
      } finally {
        setIsLoading(false);
        ...
        <div style={{ display: "flex", gap: "25px", flexWrap: "wrap", alignItems: "flex-start" }}>
          ...
          <div style={{ flex: "1 1 350px", display: "flex", flexDirection: "column", gap: "20px" }}>
            ...
            <div style={{ background: "white", padding: "25px", borderRadius: "20px", boxShadow: "0 10px 25px -5px rgba(0,0,0,0.1)", border: "1px solid #e2e8f0" }}>
              <h2 style={{ margin: "0 0 20px 0", color: "#1a202c", fontSize: "18px", display: "flex", alignItems: "center", gap: "10px", borderBottom: "2px solid #edf2f7", paddingBottom: "10px" }}>
                <span>👤</span> Input Data Baru
              </h2>
              ...
              <form onSubmit={handleTambahKaryawan} style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
                ...
                <button type="submit" disabled={isLoading} style={{ width: "100%", padding: "15px", background: isLoading ? "#a0aec0" : "#3182ce", color: "white", border: "none", borderRadius: "10px", fontWeight: "bold", cursor: isLoading ? "not-allowed" : "pointer", marginTop: "10px", boxShadow: isLoading ? "none" : "0 4px 6px rgba(49,130,206,0.3)", transition: "0.2s" }}>
                  {isLoading ? "Menyimpan..." : "➕ Simpan ke Direktori"}
                </button>
              </form>
            </div>
