...
Update terakhir: 30 Agustus 2026 (lanjutan §19: fix bug 404 `dashboard/security/page.tsx`, Parkir/Pulang jadi aksi instan tanpa modal, tampilan Daftar/Log Kendaraan Driver dibuat sama persis dengan Security, foto wajib per bagian di Inspeksi, form Servis dirombak (multi jenis + aturan foto kondisional + odometer wajib nyatu), hapus tombol "Kembali ke Portal Utama" yang buggy + mobile nav modern 4 menu, fix bug driver_status_logs ikut-ikutan berubah pas Standby/Pulang, fitur baru Admin > Update Dokumen SOP (Security/Driver/OB & CS) — SUDAH DI-COMMIT & DI-DEPLOY, lihat §20)
...
## 0. 🔴 MULAI DARI SINI — Ringkasan & Lanjutan (akhir sesi 30 Agustus 2026 — §20)

Dokumen ini di-update biar chat/sesi berikutnya langsung nyambung tanpa baca ulang semua histori di bawah. Sesi ini ada 2 batch request dari user dalam 1 percakapan: (A) fix bug 404 `dashboard/security/page.tsx` (migrasi ke `useAuthGuard`, standalone, diminta lewat instruksi terpisah sebelum batch besar), lalu (B) batch besar "sedikit perbaikan": tombol Parkir/Pulang di `parkir` jadi instan tanpa modal, tampilan Daftar/Log Kendaraan Driver disamakan persis dengan Security, foto wajib per bagian di Inspeksi Mingguan, rombak total form Servis (multi jenis servis + aturan foto kondisional + odometer wajib jadi 1 form), hapus tombol "Kembali ke Portal Utama" (bug) + mobile nav modern 4 menu buat Driver, lalu — di tengah verifikasi browser — ketemu & fix bug tambahan (`driver_status_logs` ikut ter-update walau cuma Parkir/Pulang, seharusnya cuma Keluar/Service), dan fitur baru "Update Dokumen SOP" di Admin. Detail teknis lengkap ada di **§20**.

### A. Apa yang dikerjakan sesi ini (§20)

1. **Fix bug 404 `dashboard/security/page.tsx`** (§20A) — halaman ini masih pakai cek auth manual (`localStorage` + `router.push("/dashboard")`) padahal `/dashboard` gak punya `page.tsx` fisik (404 di static export) — persis bug yang jadi motivasi `useAuthGuard.ts` dibuat (lihat komentar header file itu) tapi halaman ini sendiri gak pernah dimigrasi. Dimigrasi ke `useAuthGuard({depts:["Security"], redirectTo:"/"})`, logika bisnis (tarik daftar staf, deteksi Danru/Koordinator, filter magang) dipertahankan apa adanya.
2. **`dashboard/security/parkir`: Parkir/Standby & Pulang jadi aksi INSTAN** (§20B) — klik langsung tercatat ke `operational_vehicle_logs` tanpa modal (driver dibawa dari log terakhir kendaraan itu). Keluar & Service TETAP lewat modal (butuh Tujuan/Keperluan & siapa yang bawa). Pola sama diterapkan di `DriverArmadaPage.tsx` (driver = diri sendiri, bukan pilih dari daftar).
3. **`DriverArmadaPage.tsx` dirombak TOTAL** (§20C) — dari 1 form dropdown jadi 2 tab SAMA PERSIS gaya `dashboard/security/parkir` (Daftar Kendaraan + 4 tombol aksi cepat / Log Pergerakan Armada), CSS & struktur disalin dari security lalu disesuaikan (tanpa panel "Status Kesiagaan Driver" punya orang lain, tanpa pilih-siapa-yang-bawa di modal karena selalu diri sendiri).
4. **`DriverInspeksiPage.tsx`: foto WAJIB per bagian** (§20D) — dari 1 foto opsional global jadi 8 slot upload wajib (1 per item checklist: Ban, Rem, Lampu, Oli, dst), submit diblokir kalau ada 1 aja yang belum difoto.
5. **`DriverServisPage.tsx` dirombak total** (§20E) — jenis servis jadi multi-select (checkbox toggle, bukan pilih 1), aturan foto KONDISIONAL per jenis (Ganti Ban Luar/Dalam/Tubles/Uji Emisi/dll → wajib 1 foto sendiri-sendiri; Servis Berkala → wajib 3 foto khusus: Kendaraan+KM+Buku Servis; Ganti Oli → TANPA foto, dianggap sudah masuk Servis Berkala), odometer jadi field wajib yang MENYATU ke form utama (tombol "Catat" terpisah dihapus, odometer otomatis ikut tersimpan pas submit laporan servis).
6. **`DriverMenuPage.tsx`: hapus tombol "Kembali ke Portal Utama"** (bug — bikin user pindah ke portal sambil sesi Driver masih nempel di `localStorage`, padahal ke portal seharusnya wajib logout) **+ tambah mobile bottom-nav modern 4 menu** (Armada/Inspeksi/Servis/Keluar — pola `.mobile-nav`/`.m-nav-item` disalin dari `dashboard/security`), 3 card yang sudah ada shortcut-nya di bottom-nav disembunyikan dari grid pas mobile (`hideOnMobile`, pola sama seperti `menuSecurity`).
7. **Bug fix `driver_status_logs`** (§20F, ketemu pas verifikasi browser, BUKAN dari request awal) — sebelumnya tiap aksi Parkir/Standby & Pulang (baik dari Security maupun Driver) ikut nulis `driver_status_logs` (status kesiagaan driver berubah walau cuma kendaraan yang parkir/pulang, bukan drivernya keluar tugas). Diperbaiki: `driver_status_logs` SEKARANG cuma ke-sync pas Keluar/Service (yang beneran berarti driver sedang bertugas di luar bareng kendaraan itu) — persis sesuai koreksi eksplisit user di tengah sesi.
8. **Fitur baru: Admin > Update Dokumen SOP** (§20G) — halaman baru `/admin/sop` (upload PDF/Word/gambar, judul+deskripsi, pilih tujuan Security/Driver/OB & CS, list+filter+hapus), komponen shared baru `SopViewerPage.tsx` dipakai 3 route viewer read-only (`/dashboard/security/sop`, `/dashboard/driver/sop`, `/dashboard/ob/sop`), 1 card menu baru "SOP & Instruksi Kerja" ditambah ke ketiga dashboard. Helper upload baru `src/lib/uploadDokumen.ts` (raw file ke Cloudinary, beda dari `uploadFoto.ts` yang khusus kompres gambar). **Butuh 1 composite index Firestore baru** (`sop_documents`: `target_dept` ASC + `waktu_upload` DESC) — sudah didaftarkan di `firestore.indexes.json`, WAJIB ikut ke-deploy (`firebase deploy --only firestore:indexes`) atau query-nya bakal gagal total di production (`failed-precondition`, ketemu langsung pas testing browser).

### B. ⚠️ Insiden sesi ini: testing browser sempat nulis data sampah ke Firestore PRODUCTION

Saat verifikasi klik tombol Parkir/Standby di `DriverArmadaPage.tsx` lewat automated browser tool, pane sempat "hidden"/timeout berkali-kali tapi klik-nya TETAP kejadian di background (kadang berkali-kali/nyasar ke baris lain) — hasilnya 11 dokumen sampah di `operational_vehicle_logs` + 12 di `driver_status_logs` (semua atas nama "Amal Setiawan"/"Aplikasi Driver", rentang waktu sempit ~20:16-20:19 WITA). **Dikonfirmasi tidak ada Firestore emulator** (`src/lib/firebase.ts` connect langsung ke project `sibm-app` — sama seperti insiden serupa di §19B, project ini memang cuma punya 1 environment). Ke-23 dokumen diidentifikasi presisi by ID, dikonfirmasi ke user dulu, baru dihapus (`deleteDoc` manual satu-satu) setelah dapat izin eksplisit — diverifikasi lagi setelahnya, data balik ke kondisi sebelum testing. **Pelajaran buat sesi depan**: hindari automated click-testing pada tombol yang nulis data kalau bisa (apalagi kalau ada tanda pane "hidden"/timeout — itu sinyal klik mungkin ke-retry/nyasar), lebih aman verifikasi pakai `get_page_text`/`read_page` (baca-saja) atau, kalau harus klik tombol yang nulis data, batasi ke 1 klik per elemen dan langsung cross-check hasilnya sebelum lanjut ke elemen berikutnya.

### C. Status: SUDAH di-commit & di-deploy

- `npx tsc --noEmit` (project-wide): 0 error. `npx eslint .` (project-wide): 0 error, 104 warning — SEMUA pre-existing di file yang gak disentuh sesi ini (dicek satu-satu, gak ada yang nambah dari perubahan sesi ini).
- `npm run build`: sukses, semua route ter-generate termasuk 4 route baru (`admin/sop`, `dashboard/security/sop`, `dashboard/driver/sop`, `dashboard/ob/sop`).
- Diverifikasi langsung di browser pakai data production real: `dashboard/driver/armada` 2 tab kebaca identik sama `dashboard/security/parkir` (termasuk mobile view), mobile bottom-nav Driver 4 menu tampil benar + grid card yang ke-cover disembunyikan, `admin/sop` render form+list kosong dengan benar, `dashboard/security/sop` & `dashboard/driver/sop` render shell dengan benar (list-nya baru kebaca beneran setelah index Firestore selesai di-build pasca deploy).

### D. Yang perlu dilanjutkan

1. **`DashboardOBPage.tsx` (`/dashboard/ob`) punya bug 404 YANG SAMA PERSIS** dengan yang baru difix di §20A — baris ~124 masih `router.push("/dashboard")` (rute yang gak punya `page.tsx` fisik) kalau sesi kosong/invalid, belum dimigrasi ke `useAuthGuard`. Ditemukan gak sengaja pas baca file ini buat nambah menu SOP, BELUM difix sesi ini (di luar scope yang diminta user) — direkomendasikan buat sesi depan, polanya identik §20A (tinggal ganti manual check jadi `useAuthGuard({depts:["OB & CS"], redirectTo:"/"})`).
2. Status "Pulang" belum ditangani eksplisit di portal (`app/page.tsx`) & `admin/kendaraan/page.tsx`) — poin lama dari §19, belum berubah.
3. `admin/kendaraan/page.tsx` (riwayat servis) masih baca `foto_emisi_url` sebagai 1 foto tunggal — sekarang `DriverServisPage.tsx` bisa kirim banyak foto (`foto_detail`, object multi-key), `foto_emisi_url` cuma diisi foto PERTAMA buat kompatibilitas mundur. Kalau admin perlu lihat SEMUA foto servis (bukan cuma 1), `admin/kendaraan/page.tsx` perlu diupdate buat baca & tampilkan `foto_detail` — belum dikerjakan sesi ini (di luar scope).
4. Poin lama dari §17/§18 yang masih sama (belum berubah): `GROUPED_PATROLI`/`DATA_SECURITY` hardcode duplikat, `PengaturanJadwalSecurity.tsx`/`PaketPage.tsx` dead code di `components/pages/`, `NotifikasiChecklistListener.tsx` dead code (digantikan `ChecklistOBBanner`), `security_magang_directory` belum ada halaman admin, bug filter logo `admin/qr-manager`, kolom Odo Meter `admin/uji-emisi`, CS belum punya halaman terpisah dari OB, audit performa `onSnapshot` tanpa `limit()`, bug leak `DeepCleaningPage.tsx`, migrasi struktur folder `admin/*` & portal utama, `admin/atk` redirect ke route 404 (`/shift-checkin`).
5. **User perlu paste kode HTML EmailJS Code Editor** (§18C) & cek alarm suara banner (§18D) — 2 poin lanjutan dari §18 ini BELUM dikonfirmasi user, masih menunggu.
