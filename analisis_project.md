...
### A. Apa yang dikerjakan sesi ini (§21)

1. **Dropdown Unit Bisnis 11 PT** (§21A) — konstanta baru `src/lib/unitBisnis.ts` (`DAFTAR_UNIT_BISNIS`, 11 PT + prefix "PT") dipakai di: `admin/karyawan/page.tsx` (field "Unit Bisnis / Departemen", WAJIB), `admin/kendaraan/page.tsx` (field "Unit Bisnis", opsional), `BukuTamuSecurity.tsx` tab Magang (field "Unit Bisnis"). Data existing yang typo/gak match persis (mis. `master_kendaraan` ada "PT Samudera Agencies **Indoensia**", "PT **SIL**kargo Indonesia", "PT Samudera **Perdana**" tanpa "PAD") TIDAK diubah otomatis — dropdown cuma gak nunjuk ke opsi manapun sampai user re-select & save manual lewat form Edit.
2. **Ketemu masalah pas verifikasi browser, user dikonsultasikan via AskUserQuestion**: data `employees_directory.departemen` (Master Data Karyawan) ternyata BUKAN cuma isi nama PT — banyak entry isinya kategori internal ("Security", "Driver", "Cleaning", "Office Boy", "Building Management" — termasuk record Reza sendiri). Kalau dropdown dikunci ketat 11 PT doang, staf-staf ini gak akan punya opsi yang pas kalau datanya diedit lagi. User pilih opsi "11 PT + kategori internal". Ditambah konstanta `DAFTAR_DEPARTEMEN_INTERNAL` (isi: Admin GA, Management, OB & CS, Security, Driver, QHSE — SAMA PERSIS dengan enum `departemen` di `users_master` yang dipakai `useAuthGuard`, biar konsisten satu sistem). Dropdown `admin/karyawan` & `BukuTamuSecurity` (Magang) pakai `<optgroup>` 2 grup: "Unit Bisnis (PT)" dan "Departemen Internal Gedung". `admin/kendaraan` TETAP ketat 11 PT saja (kendaraan fisik jelas milik 1 PT tertentu, gak ada kasus "kendaraan Security" dkk).
3. **Field SBO di portal (`app/page.tsx`) dijadiin read-only** — sebelumnya field "Unit Bisnis / Departemen" di form Lapor Bahaya (SBO) satu-satunya yang masih bisa diedit manual walau sudah ada auto-fill dari lookup nama pelapor (`handleNameChangeSbo`), gak konsisten sama field sejenis di form ATK & Overtime yang sudah `readOnly`. Disamakan jadi `readOnly` juga.
4. **Sinkronisasi otomatis Karyawan → Kendaraan** (§21B, di `BukuTamuSecurity.tsx`) — pas Security check-in seorang "Karyawan" di Buku Tamu Digital, fungsi baru `syncKendaraanStandby()` dipanggil: cari kendaraan di `master_kendaraan` yang `plat_nomor`-nya cocok (dinormalisasi via helper baru `src/lib/platUtils.ts` → `normalizePlat()`, biar "DD 1234 AB" = "dd-1234-ab") dengan `plat_kendaraan` milik karyawan itu (field di Master Data Karyawan). Kalau ketemu DAN kendaraan itu belum berstatus Standby (dicek lewat query `operational_vehicle_logs` terbaru, pakai index existing `kendaraan ASC + waktu_catat DESC` — sudah ada di `firestore.indexes.json`, gak perlu index baru), tulis 1 log baru status "Tiba di Kantor (Standby)" dengan `petugas_security: "<nama> (Auto-Sync Buku Tamu)"`. Karyawan tanpa plat kendaraan (field kosong) gak memicu apa-apa.
5. **Sinkronisasi otomatis Kendaraan → Karyawan** (§21C, di `dashboard/security/parkir/page.tsx` DAN `DriverArmadaPage.tsx` — 2 tempat yang sama-sama punya tombol instan "Parkir/Standby") — pas kendaraan di-set Standby, fungsi baru `syncKaryawanHadir()` dipanggil: cari karyawan di `employees_directory` yang `plat_kendaraan`-nya cocok (normalisasi sama) dengan `plat_nomor` kendaraan itu. Kalau ketemu DAN karyawan itu belum tercatat "Di Dalam Area" jenis "Karyawan" di `security_visitor_logs` (dicek dari state yang sudah di-subscribe live, BUKAN query baru — sengaja hindari query 3-filter equality yang berisiko butuh composite index belum-ada), tulis 1 entry check-in baru otomatis (`pic_bertugas: "<petugas> (Auto-Sync Kendaraan)"`, `tujuan: "Bekerja / Operasional"`). Kendaraan tanpa PIC yang match karyawan manapun (lewat plat) gak memicu apa-apa.
6. **Arah yang SENGAJA TIDAK diimplementasi** (sesuai permintaan user yang spesifik cuma 2 arah di atas): checkout Karyawan tidak otomatis bikin kendaraan "Pulang", dan kendaraan "Pulang"/"Keluar"/"Service" tidak otomatis checkout karyawan. Kalau user mau tambahan ini nanti, tinggal cermin pola yang sama.

### B. Kenapa 2 arah sinkron ini gak saling infinite-loop

Baik `syncKendaraanStandby()` maupun `syncKaryawanHadir()` HANYA dipanggil langsung dari 1 event klik user (bukan dari `onSnapshot` listener yang bereaksi ke perubahan collection), jadi walau Buku Tamu bisa memicu tulis ke `operational_vehicle_logs` dan Parkir/DriverArmada bisa memicu tulis ke `security_visitor_logs`, gak ada kode yang "mendengarkan" collection itu lalu otomatis mancing balik ke arah lain — rantainya berhenti begitu 1 klik user selesai diproses. Guard "kalau sudah Standby/sudah Di Dalam Area, skip" di kedua fungsi juga mencegah dobel-catat kalau user klik berkali-kali.

### C. Apa yang dikerjakan sesi ini (§22 — batch kedua)

1. **Fix "Upload dokumen gagal" di Admin > Update Dokumen SOP** (§22A) — root cause DIDUGA file dokumen (PDF/Word scan SOP) kena batas ukuran unsigned-upload Cloudinary (preset `sibm_storage` dicek via Admin API: gak ada `allowed_formats`/pembatasan format, jadi bukan soal tipe file — paling mungkin ukuran, apalagi foto kamera yang SELAMA INI selalu lolos karena dikompres canvas ke max lebar 600px, sedangkan dokumen SOP diupload RAW tanpa kompresi). `src/lib/uploadDokumen.ts` diperbaiki: (a) `res.json()` dibaca DULU sebelum cek `res.ok`, error di-throw pakai `data.error.message` ASLI dari Cloudinary (sebelumnya generic "Upload dokumen gagal" doang, gak kebaca alasannya di UI) — kalau ada eror lagi ke depan, pesannya sekarang informatif; (b) konstanta baru `MAX_UKURAN_DOKUMEN_MB = 10`. `admin/sop/page.tsx` — `handleFileChange` sekarang cek ukuran file DULU sebelum coba upload (toast jelas "Ukuran file maksimal 10MB..." kalau kelewat, bukan nunggu Cloudinary nolak), dan toast error sekarang nampilin pesan asli (`err.message`) bukan teks generik.
2. **Card "Menu Cepat" portal gak proporsional** (§22B) — root cause: grid pakai `repeat(auto-fit, minmax(150px, 1fr))`, yang bikin kartu SENDIRIAN di baris terakhir (misal "Bahaya SBO" kalau cuma dia yang gak muat di baris sebelumnya) ikut melebar `1fr` ngisi kolom kosong sisanya — gak sama lebar dengan kartu-kartu lain. Diganti kolom TETAP lewat class baru `.menu-cepat-grid` (2 kolom di mobile, 3 kolom di desktop lewat `@media (min-width: 640px)`) — kolom tetap gak pernah stretch kartu buat ngisi ruang kosong. Sekalian `.qa-card` dikasih `height: 100%` dan `.desktop-only-hide` dikasih `height: 100%` biar kartu yang dibungkus wrapper (ATK/Kerusakan/SBO) ikut setinggi kartu lain dalam baris yang sama (sebelumnya bisa lebih pendek karena wrapper `display:block`-nya gak ikut stretch ke kartu di dalamnya).
3. **Label plot OB & CS di "Tim Bertugas Hari Ini" gak jelas** (§22C) — sebelumnya `sub: "OB · " + lokasi[0] + " +N"` (cuma nampilin lantai PERTAMA, sisanya diringkas jadi "+2" dst — persis yang dikomplain user di screenshot). Diganti `sub: "OB · " + lokasi.join(", ")` — semua nama lantai yang di-plot ditulis lengkap (pola `.join(", ")` ini sebenarnya SUDAH dipakai di tempat lain di file yang sama, "Plot Besok" baris ~1201, cuma "Tim Bertugas Hari Ini" belum ikut dipakein).

### D. Status: SUDAH di-commit & di-deploy (§21 + §22 sekaligus)

- `npx tsc --noEmit` (project-wide, setelah SEMUA perubahan §21+§22): 0 error. `npx eslint .` (project-wide): 0 error, 104 warning — SAMA PERSIS baseline, dicek satu-satu gak nambah.
- `npm run build`: sukses, semua 39 route ter-generate, gak ada error.
- Diverifikasi di browser: dropdown 11 PT (§21) dicek render benar (baca-saja, data production real). Fix §22B & §22C dicek visual via screenshot browser (desktop: 3 kartu per baris, semua ukuran sama; mobile: 2 kolom; "Tim Bertugas Hari Ini" sekarang nampilin "OB · Lantai 5, Basement, Lantai 1" dkk, lengkap bukan "+N"). Fix §22A (upload dokumen) BELUM ketest langsung upload file beneran (gak ada file SOP nyata buat ditest, dan sengaja hindari upload sampah ke Cloudinary/Firestore production) — tapi root-cause analysis dikonfirmasi via `curl` langsung ke Cloudinary API pakai kredensial project (preset `sibm_storage` dicek, PDF & docx kecil UPLOAD SUKSES lewat curl, jadi bukan soal tipe file/preset-config, konsisten dengan dugaan soal ukuran file asli yang lebih besar dari test).
- **Sinkronisasi 2 arah §21B/§21C juga BELUM ketest end-to-end interaktif** (masih sama seperti status sebelumnya, sengaja ditunda hindari nulis data sampah ke production) — kalau ternyata ada masalah pas dipakai beneran, laporkan di sesi berikutnya.
- Sesuai instruksi eksplisit user ("setelah itu lansung saja commit/push/deploy"): **sudah di-commit, di-push (dev→main), dan di-deploy ke Firebase Hosting.**

### E. Yang perlu dilanjutkan

1. Root cause §22A (ukuran file) belum 100% dikonfirmasi karena gak ada file SOP asli buat ditest — kalau user masih dapat error pas upload file di bawah 10MB, perlu digali lagi (kemungkinan lain: format file gak didukung `/auto/upload`, koneksi lambat/timeout, dll).
2. Rekonsiliasi data lama di `master_kendaraan.unit_bisnis` yang typo dari 3 nama PT (lihat §21A poin 1) — gak urgent (data gak rusak), tapi kalau mau seragam sebaiknya di-edit manual lewat form Admin > Master Data Kendaraan.
3. Sinkronisasi §21B/§21C belum ketest end-to-end pakai data production real (baru dicek logic-nya lewat baca kode + tsc/eslint/build) — disarankan ditest hati-hati (1 klik, cross-check, siap bersihkan) di sesi berikutnya atau langsung user.
4. `DashboardOBPage.tsx` (`/dashboard/ob`) masih punya bug 404 (belum dimigrasi ke `useAuthGuard`) — poin lama dari §20D, belum berubah.
5. Status "Pulang" belum ditangani eksplisit di portal (`app/page.tsx`) & `admin/kendaraan/page.tsx` — poin lama dari §19, belum berubah.
5. `admin/kendaraan/page.tsx` (riwayat servis) masih baca `foto_emisi_url` sebagai 1 foto tunggal, belum baca `foto_detail` multi-foto — poin lama dari §20D, belum berubah.
