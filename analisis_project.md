...
## 56. Fondasi Ekspansi Multi-Daerah: Super Admin vs Admin Daerah (25 September 2026, lanjutan langsung §55)

User mau upgrade SIBM buat dipakai GA di seluruh Indonesia -- bakal ada Admin per daerah, sementara user sendiri (Super Admin) tetap akses penuh semua daerah, dan Admin Daerah cuma boleh cek data daerahnya sendiri. Ditanya balik dulu "mulai dari mana" (exploratory) sebelum dikerjakan -- jawabannya: masalah intinya BUKAN cuma nambah filter tampilan, tapi `firestore.rules` yang sekarang `if col in [...]` itu TIDAK ngecek siapa yang login sama sekali, jadi kalau cuma filter React, admin daerah lain tetap bisa akses data daerah lain langsung lewat Firestore SDK. User bilang "langsung jalankan saja", minta dijelaskan skema+rules SETELAH selesai.

**Temuan penting sebelum mulai**: dicek ulang `src/app/page.tsx` (bukan asumsi dari plan lama) -- ternyata migrasi Firebase Auth SUDAH selesai di sesi sebelumnya (`users_master` docs SUDAH pakai Firebase Auth UID sebagai doc ID, login sudah pakai `signInWithEmailAndPassword` sungguhan). Ini artinya fondasi paling penting (rules bisa `get()` profil user via UID) SUDAH ADA -- gak perlu migrasi ulang dari nol seperti dikira di awal.

### 56A. Skema Data
`users_master` dapat field baru `daerah: string`. Nilai sentinel **`'PUSAT'`** = Super Admin (akses & kelola SEMUA wilayah tanpa batas). Nilai lain (mis. `"Makassar"`, `"Jakarta"`) = Admin Daerah, discope ke wilayah itu saja. Dipilih pola "1 field dengan nilai sentinel" (bukan role terpisah "Super Admin" vs "Admin") supaya gak nambah percabangan role baru -- role tetap "Administrator" seperti sekarang, cuma `daerah`-nya yang membedakan tingkat aksesnya.

### 56B. Firestore Rules
2 helper baru di `firestore.rules`: `myDaerah()` (baca field `daerah` dari profil user yang login, via `get()` ke `users_master/{uid}` yang SUDAH ada polanya dari `myDept()`/`isAdmin()`) dan `isSuperAdmin()` (`isAdmin() && myDaerah() == 'PUSAT'`). Rule `users_master` dipecah jadi `create`/`update`/`delete` terpisah (sebelumnya 1 `write` gabungan) -- Super Admin bebas kelola akun mana pun, Admin Daerah CUMA boleh buat/ubah/hapus akun yang `daerah`-nya SAMA PERSIS dengan `daerah` dia sendiri (gak bisa bikin akun utk daerah lain, gak bisa "pindahkan" akun ke daerah lain).

**Bug kritis ketemu & langsung difix sebelum kepakai**: percobaan pertama pakai `myProfile().daerah` (dot-notation) -- karena field ini BARU dan belum ada di akun manapun (migrasi belum jalan), akses field yang gak ada lewat dot-notation bikin RULE ERROR (dianggap deny), bukan graceful null. Ini berarti fitur manajemen user (`admin/users`) bakal langsung gagal total buat SEMUA orang begitu rules ini di-deploy, sebelum sempat dipakai siapa pun. Ketauan lewat analisis manual (bukan lewat laporan user), langsung diganti ke `.get('daerah', '')` (method Map yang null-safe di Firestore Rules) dan di-redeploy ulang SEBELUM kejadian nyata. Pelajaran: field baru di rules HARUS pakai `.get(key, default)`, gak boleh dot-notation langsung, sampai migrasi backfill-nya beneran selesai di semua dokumen.

### 56C. `admin/users` -- Manajemen Akun Per Wilayah
Form tambah/edit akun dapat field "Wilayah/Daerah" + checkbox "Jadikan Super Admin" -- **cuma muncul kalau yang login sendiri Super Admin** (dicek client-side via `isSuperAdmin(role, daerah)` helper baru di `useAuthGuard.ts`, DAN ditegakkan lagi di rules kalau ada yang coba akali lewat DevTools). Admin Daerah yang bikin akun baru otomatis kepasang `daerah` sesuai wilayahnya sendiri (gak ada pilihan lain). Daftar akun juga di-scope tampilannya: Admin Daerah cuma LIHAT akun wilayahnya sendiri (Super Admin lihat semua) -- **catatan jujur**: ini scoping TAMPILAN saja, bukan penegakan keamanan (rules `users_master` tetap `read: if true` seperti sebelumnya, karena profil staf memang sudah dianggap data publik dalam app ini sejak awal buat kontak/foto profil) -- keamanan sungguhan ada di rules create/update/delete yang sudah membatasi SIAPA BOLEH MENULIS akun mana.

### 56D. `useAuthGuard.ts` & Login
`AuthSession` dapat field `daerah`. Helper baru `isSuperAdmin(role, daerah)` (mirror persis logika `isSuperAdmin()` di rules). `pic_daerah` disimpan ke localStorage saat login (`src/app/page.tsx`) bareng `pic_nama`/`pic_dept`/`pic_role` yang sudah ada.

### 56E. `scripts/migrate-add-daerah.mjs` (BARU, sekali jalan manual)
Backfill field `daerah` ke akun lama yang belum punya: default `"Makassar"` untuk semua, KECUALI 1 email (di-hardcode di script, email Reza) yang diset `"PUSAT"` jadi Super Admin pertama. Idempotent (skip akun yang sudah punya `daerah`, aman dijalankan ulang). **User PERLU jalankan script ini secara manual** (pola sama seperti `migrate-users-to-auth.mjs` sebelumnya) supaya fitur Super Admin beneran aktif -- SEBELUM itu, semua akun (termasuk Reza) dianggap Admin Daerah dengan `daerah=''`, dan setelah migrasi jalan, **Reza perlu logout+login ulang** karena `pic_daerah` di localStorage cuma di-set saat login, gak otomatis ke-refresh dari sesi yang sedang aktif.

### 56F. INI FASE 1 (Fondasi Akun) -- Data Operasional BELUM Di-scope
Sengaja BERTAHAP, bukan sekaligus semua collection: scoping data operasional (laporan patroli, checklist, dll per daerah) butuh 2 hal lagi yang BELUM dikerjakan -- (1) setiap collection transaksional perlu ditandai `daerah` saat ditulis (staf bikin laporan otomatis kepasang wilayahnya), dan (2) SEMUA query admin monitoring (`admin/monitor-*`) yang sekarang `onSnapshot(collection(db, "..."))` TANPA filter perlu ditambah `.where("daerah", "==", ...)` -- kalau ENGGAK, rules yang membatasi baca per-daerah bakal bikin query lama ERROR TOTAL (Firestore menolak query "list" yang gak match filter rule, bukan diam-diam nyaring hasil). Karena baru ada 1 daerah aktif di production sekarang (Makassar) dan resiko regresi ke fitur yang dipakai aktif tiap hari, fase ini SENGAJA ditunda sampai memang ada Admin Daerah kedua yang mau onboarding.
